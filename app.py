            ...
            with st.container():
                ...
                with shot_col1:
                    ...
                    if image_bytes:
                        ...
                        st.caption((shot.room_type or "").replace("_", " ").title())
                    else:
...
if st.session_state.extracted_details is not None:
    ...
    # --- Image-Detected Features — 4 columns ---
    details = st.session_state.extracted_details
        ...
        for i in range(0, len(rooms), 4):
            ...
            for col_idx, room in enumerate(rooms[i:i+4]):
                with cols[col_idx]:
                    ...
                    candidates = grouped_candidates[room]
                    for candidate in candidates:
...
if st.session_state.marketing_results:
    ...
    if res.get("headline"):
        ...
        st.caption("Use in Zillow's What's Special section, email subject lines, and social captions.")
    ...
    if "compliance_results" in res:
        with st.expander("⚖️ Fair Housing Compliance Review", expanded=False):
            st.caption(
                "Every asset reviewed for fair housing compliance. "
    ...
    if st.session_state.extracted_details and st.session_state.extracted_details.images:
        with st.expander("📷 Photo Enhancement Preview", expanded=False):
            ...
            enhanced_lookup_photos = build_image_lookup(st.session_state.enhanced_images or [])

            for img in st.session_state.extracted_details.images:
                ...
                with photo_col2:
                    st.caption("Enhanced")
                    enhanced_bytes = enhanced_lookup_photos.get(img.filename)
                    if enhanced_bytes:
