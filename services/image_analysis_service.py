import asyncio
from typing import List

from chains.image_extraction_chain import extract_property_image
from models.property_image import PropertyImage


async def analyze_single_image_async(image_bytes, filename, image_id=None):
    loop = asyncio.get_event_loop()

    return await loop.run_in_executor(
        None,
        extract_property_image,
        image_bytes,
        filename,
        image_id,
    )


async def analyze_property_images(uploaded_images) -> List[PropertyImage]:
    """
    Analyze multiple uploaded property images in parallel.

    uploaded_images should be a list of tuples:
    [(image_bytes, filename), ...]
    """

    tasks = []

    for i, (image_bytes, filename) in enumerate(uploaded_images):
        image_id = f"img_{i+1:03}"

        tasks.append(
            analyze_single_image_async(
                image_bytes=image_bytes,
                filename=filename,
                image_id=image_id,
            )
        )

    results = await asyncio.gather(*tasks)

    return results